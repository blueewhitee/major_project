...
{
  "todayCategories":   [ { "category", "categoryKey", "seconds", "percent", "formattedTime" }, ... ],
  "allTimeCategories": [ { "category", "categoryKey", "seconds", "percent", "formattedTime" }, ... ],
  "transitions":       [ { "fromCategory", "fromCategoryKey", "toCategory", "toCategoryKey",
                            "strength", "count", "description" }, ... ],
  "stale": bool,
  "staleMinutes": float | null,
  "lastUpdated": str | null,
  "generatedAt": str
}
"""
import json
...
def _get_description(from_cat: str, to_cat: str) -> str:
    ...
    if desc:
        return desc
    from_name = from_cat.capitalize()
    to_name   = to_cat.capitalize()
    return f"Shifting from {from_name} to {to_name}."
...
def _infer_category_from_context(row: sqlite3.Row) -> str:
    ...
    # Final deterministic fallback by productive/distractive class
...
def _top8_with_percent(cat_map: dict[str, float]) -> list[dict]:
    ...
    rows = sorted(
        (
            {
                "category":      _label(cat),
                "categoryKey":   cat,               # lowercase key for frontend colour lookup
...
def _compute_transitions(conn) -> list[dict]:
    """
    Walk all-time events in chronological order and find consecutive
    PRODUCTIVE → DISTRACTIVE switches that occur within 10 minutes.
    Return the top-5 pairs ranked by frequency.
    """
    rows = conn.execute(
        """
        SELECT classification, category, source, app, url, title,
               COALESCE(event_timestamp, timestamp) AS ts
        FROM events
        WHERE classification IN ('PRODUCTIVE', 'DISTRACTIVE')
    ...
    for i in range(len(rows) - 1):
        ...
        try:
            ...
            if gap_seconds < 0 or gap_seconds > 600:   # must be 0–10 minutes apart
                continue
        except Exception:
            continue
...
def main() -> None:
    ...
    db_path   = sys.argv[1]
    start_iso = sys.argv[2]   # today's midnight
    end_iso   = sys.argv[3]   # now

    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row

    # ── Guard: category column may not exist on old DBs ──────────
    ...
    all_rows = conn.execute(
        """
        SELECT event_timestamp, timestamp, duration_seconds, category,
               source, app, url, title, classification
        FROM events
        """
    ).fetchall()
    ...
    # ── Transitions (all-time) ───────────────────────────────────
    transitions = _compute_transitions(conn)

    conn.close()

    # ── Aggregate duration per category ─────────────────────────
    def _aggregate(rows) -> dict[str, float]:
    ...
    all_time_map = _aggregate(all_rows)
    ...
    print(json.dumps({
        ...
        "allTimeCategories": _top8_with_percent(all_time_map),
